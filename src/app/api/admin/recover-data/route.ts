import { NextRequest, NextResponse } from 'next/server';
import { DataService } from '@/lib/dataService';

// Force dynamic rendering
export const dynamic = 'force-dynamic';

// Helper function to check admin access
const checkAdminAccess = async (request: NextRequest): Promise<boolean> => {
  try {
    // Check if Supabase environment variables are available
    if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) {
      console.log('Supabase environment variables not available, allowing admin access for local development');
      return true;
    }

    // Dynamically import Supabase client only when needed
    const { supabase } = await import('@/lib/supabaseClient');
    
    // Get the authorization header
    let token = request.headers.get('authorization');
    
    if (token && token.startsWith('Bearer ')) {
      token = token.substring(7);
    }
    
    // Also check for access token in cookies
    const accessToken = request.cookies.get('sb-access-token')?.value;
    if (!token && accessToken) {
      token = accessToken;
    }
    
    if (!token) {
      return false;
    }

    // Verify the session with Supabase
    const { data: { user }, error } = await supabase.auth.getUser(token);

    if (error || !user) {
      return false;
    }

    // Check if user has admin privileges
    return user.user_metadata?.isAdmin || false;
  } catch (error) {
    console.error('Admin access check error:', error);
    if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) {
      return true;
    }
    return false;
  }
};

export async function POST(request: NextRequest) {
  try {
    // Check admin access
    const isAdmin = await checkAdminAccess(request);
    
    if (!isAdmin) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }

    console.log('=== DATA RECOVERY START ===');
    
    // Check environment
    const isProduction = process.env.NODE_ENV === 'production' || process.env.VERCEL === '1';
    const hasKvConfig = !!(process.env.KV_REST_API_URL && process.env.KV_REST_API_TOKEN);
    
    console.log('Environment check:', { isProduction, hasKvConfig });
    
    if (!isProduction) {
      return NextResponse.json({
        error: 'This endpoint only works in production',
        environment: { isProduction, hasKvConfig }
      }, { status: 400 });
    }
    
    if (!hasKvConfig) {
      return NextResponse.json({
        error: 'KV not configured - cannot recover data',
        environment: { isProduction, hasKvConfig }
      }, { status: 400 });
    }
    
    // Force initialize all data in KV
    console.log('Initializing all data in KV...');
    await DataService.initializeDefaultData();
    
    // Get current data
    const timetable = await DataService.getTimetable();
    const bookings = await DataService.getBookings();
    const users = await DataService.getUsers();
    
    console.log('Data recovery complete:', {
      timetableCount: timetable.length,
      bookingsCount: bookings.length,
      usersCount: users.length
    });
    
    // Create some sample bookings to test persistence
    const sampleBookings = [
      {
        id: 'recovery-booking-1',
        userId: 'user-1',
        userName: 'John Doe',
        classId: 'mon-630',
        className: 'Beginner Muay Thai',
        day: 'Monday',
        time: '18:30',
        endTime: '19:30',
        date: new Date().toISOString(),
        createdAt: new Date().toISOString()
      },
      {
        id: 'recovery-booking-2',
        userId: 'user-2',
        userName: 'Jane Smith',
        classId: 'tue-630',
        className: 'Beginner Muay Thai',
        day: 'Tuesday',
        time: '18:30',
        endTime: '19:30',
        date: new Date().toISOString(),
        createdAt: new Date().toISOString()
      }
    ];
    
    console.log('Adding sample bookings...');
    for (const booking of sampleBookings) {
      await DataService.addBooking(booking);
    }
    
    // Verify final state
    const finalBookings = await DataService.getBookings();
    
    console.log('=== DATA RECOVERY END ===');
    
    return NextResponse.json({
      message: 'Data recovery completed successfully',
      environment: { isProduction, hasKvConfig },
      data: {
        timetableCount: timetable.length,
        bookingsCount: finalBookings.length,
        usersCount: users.length,
        sampleBookingsAdded: sampleBookings.length
      },
      sampleData: {
        timetable: timetable.slice(0, 2),
        bookings: finalBookings.slice(-2),
        users: users.slice(0, 2)
      }
    });
    
  } catch (error) {
    console.error('Data recovery error:', error);
    
    const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
    
    return NextResponse.json(
      { 
        error: `Data recovery failed: ${errorMessage}`,
        details: error instanceof Error ? error.stack : undefined
      },
      { status: 500 }
    );
  }
}
